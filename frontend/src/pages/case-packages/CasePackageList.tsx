...
const CasePackageList: React.FC = () => {
  ...
  const loadData = async (params?: Partial<CasePackageQueryParams>) => {
    ...
    try {
      ...
      const response = await casePackageService.getCasePackageList(mergedParams);
      
      console.log('LoadData response:', response);
      console.log('Response content:', response.content);
      console.log('Response content length:', response.content?.length);
      
      const newDataSource = response.content || [];
      console.log('Setting dataSource to:', newDataSource);
  ...
  const calculateStatistics = (data: CasePackage[]) => {
    const stats = {
      ...
      processingPackages: data.filter(p => p.status === 'IN_PROGRESS').length,
      completedPackages: data.filter(p => p.status === 'COMPLETED').length,
      totalAmount: data.reduce((sum, p) => sum + (p.totalAmount || 0), 0),
      recoveredAmount: data.reduce((sum, p) => sum + ((p as any).recoveredAmount || 0), 0),
      avgRecoveryRate: 0
    };
  ...
  const columns: ColumnsType<CasePackage> = [
      ...
      render: (record: CasePackage) => (
        <div>
          <div style={{ fontWeight: 'bold', marginBottom: 4 }}>
            {record.packageName || `案件包${record.id}`}
          </div>
          <div style={{ fontSize: '12px', color: '#666', marginBottom: 2 }}>
            编号: {record.packageCode || `PKG${String(record.id).padStart(3, '0')}`}
          </div>
          ...
          {record.publishedAt && (
            <div style={{ fontSize: '12px', color: '#666' }}>
              发布时间: {dayjs(record.publishedAt).format('YYYY-MM-DD')}
            </div>
          )}
        </div>
      )
    ...
    },
    {
      ...
      render: (record: CasePackage) => (
        <div>
          <div>总案件: <strong>{record.caseCount || 0}</strong></div>
          <div>已分配: <span style={{ color: '#1890ff' }}>{(record as any).assignedCases || 0}</span></div>
          <div>处理中: <span style={{ color: '#faad14' }}>{(record as any).processingCases || 0}</span></div>
          <div>已完成: <span style={{ color: '#52c41a' }}>{(record as any).completedCases || 0}</span></div>
        </div>
      )
    },
    {
      ...
      render: (record: CasePackage) => (
        <div>
          ...
          <div>已回款: <span style={{ color: '#52c41a' }}>¥{((record as any).recoveredAmount || 0).toLocaleString()}</span></div>
          <div>
            回款率: <strong style={{ color: (record as any).recoveryRate > 50 ? '#52c41a' : '#faad14' }}>
              {(record as any).recoveryRate ? (record as any).recoveryRate.toFixed(1) : 0}%
            </strong>
          </div>
        </div>
      )
      ...
      render: (record: CasePackage) => (
        <div>
          ...
          {record.expectedDisposalDays && (
            <div>处置周期: {record.expectedDisposalDays}天</div>
          )}
          {record.preferredDisposalMethods && (
            <div>
              处置方式: {record.preferredDisposalMethods.split(',').map((method: string) => {
    ...
    },
    {
      ...
      width: 120,
      render: (record: CasePackage) => {
        const percent = record.caseCount > 0 ? 
          (((record as any).completedCases || 0) / record.caseCount) * 100 : 0;
        ...
        return (
          <div>
            ...
            <div style={{ fontSize: '12px', color: '#666', marginTop: 4 }}>
              {(record as any).completedCases || 0}/{record.caseCount || 0}
            </div>
          </div>
        );
      }
